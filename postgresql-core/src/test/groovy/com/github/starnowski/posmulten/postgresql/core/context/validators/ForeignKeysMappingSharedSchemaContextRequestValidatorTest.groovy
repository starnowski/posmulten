...
class ForeignKeysMappingSharedSchemaContextRequestValidatorTest extends Specification {
    ...
    def "should throw exception when foreign keys mapping does not match to defined primary keys for tests parameters: #schema, #foreignKeysTable, #primaryKeysTable, #foreignKeysMapping, #primayKeyTypeDefinition"()
    {
        ...
        where:
            ...
            null                |   "posts"             |   "comments"          |   [comment_id: "comment_id", comment_user: "user_id"]         |   [comment_id: null, user: null]      ||  "There is mismatch between foreign keys column mapping (comment_id, user_id) in posts table and primary keys column declaration (comment_id, user) for comments table"
            null                |   "posts"             |   "comments"          |   [comment_id: "id", comment_user: "user_id"]                 |   [comment_id: null, user: null]      ||  "There is mismatch between foreign keys column mapping (id, user_id) in posts table and primary keys column declaration (comment_id, user) for comments table"
            ...
            "public"            |   "posts"             |   "comments"          |   [comment_id: "comment_id", comment_user: "user_id"]         |   [comment_id: null, user: null]      ||  "There is mismatch between foreign keys column mapping (comment_id, user_id) in public.posts table and primary keys column declaration (comment_id, user) for public.comments table"
            "public"            |   "posts"             |   "comments"          |   [comment_id: "id", comment_user: "user_id"]                 |   [comment_id: null, user: null]      ||  "There is mismatch between foreign keys column mapping (id, user_id) in public.posts table and primary keys column declaration (comment_id, user) for public.comments table"
