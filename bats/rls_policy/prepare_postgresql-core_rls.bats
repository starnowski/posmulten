...
@test "Create rls policy with permission commands UPDATE and WITH CHECK expression" {
  #given
  export PGPASSWORD=postgres_posmulten
  export TEST_SCRIPT_DATABASE_USER='"postgresql-core-user"'
  cat << SQL > "$BATS_TMPDIR/$TMP_SQL_FILE"
    CREATE POLICY users_policy ON
    ${DATABASE_TESTS_SCHEMA_NAME}.users FOR UPDATE TO ${TEST_SCRIPT_DATABASE_USER} WITH CHECK (tenant_id = 'xxxx');
SQL
...
@test "should not be able to create rls policy with permission commands SELECT and WITH CHECK expression" {
  #given
  export PGPASSWORD=postgres_posmulten
  export TEST_SCRIPT_DATABASE_USER='"postgresql-core-user"'
  cat << SQL > "$BATS_TMPDIR/$TMP_SQL_FILE"
    CREATE POLICY users_policy ON
    ${DATABASE_TESTS_SCHEMA_NAME}.users FOR SELECT TO ${TEST_SCRIPT_DATABASE_USER} WITH CHECK (tenant_id = 'xxxx');
SQL
...
@test "should not be able to create rls policy with permission commands DELETE and WITH CHECK expression" {
  #given
  export PGPASSWORD=postgres_posmulten
  export TEST_SCRIPT_DATABASE_USER='"postgresql-core-user"'
  cat << SQL > "$BATS_TMPDIR/$TMP_SQL_FILE"
    CREATE POLICY users_policy ON
    ${DATABASE_TESTS_SCHEMA_NAME}.users FOR DELETE TO ${TEST_SCRIPT_DATABASE_USER} WITH CHECK (tenant_id = 'xxxx');
SQL
...
@test "should not be able to create rls policy with permission commands INSERT and WITH CHECK expression" {
  ...
  cat << SQL > "$BATS_TMPDIR/$TMP_SQL_FILE"
    CREATE POLICY users_policy ON
    ${DATABASE_TESTS_SCHEMA_NAME}.users FOR INSERT TO ${TEST_SCRIPT_DATABASE_USER} WITH CHECK (tenant_id = 'xxxx');
SQL
