...
public abstract class AbstractCustomDefinitionEntryMapper<T extends AbstractCustomDefinitionEntry<T>> implements IConfigurationMapper<com.github.starnowski.posmulten.configuration.core.model.CustomDefinitionEntry, T> {
    @Override
    public T map(com.github.starnowski.posmulten.configuration.core.model.CustomDefinitionEntry input) {
        return input == null ? null : createNewInstanceOfType().setCreationScript(input.getCreationScript()).setDropScript(input.getDropScript())
    ...
    abstract protected T createNewInstanceOfType();
}
